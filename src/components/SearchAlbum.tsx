...
export default function SearchAlbum(){
    ...
    return <>
        <div className="flex flex-col justify-start items-center relative bottom-16 w-[340px] ">
            ...
            <Droppable droppableId={String("search")}>
                {(provided, snapshot) => <div ref={provided.innerRef} {...provided.droppableProps} className="flex flex-row flex-wrap justify-start gap-1 bg-slate-300 bg-opacity-10 min-h-0 max-h-[550px] overflow-y-scroll">
                {   
                    data?.pages.map((page, row) => (
                            page?.data.map((album : albumType, col : number) => {
